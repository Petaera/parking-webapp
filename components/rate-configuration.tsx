...
import { getLots, getSlabByLotId, Slab, TimeRangeType, VehicleType } from "@/lib/firestore-service"
...
export default function RateConfiguration() {
  ...
  // Function to fetch lots from Firebase
  const fetchLots = async () => {
    try {
      setIsLoadingLots(true)
      const lotsData = await getLots()
      setLots(lotsData)
      setSelectedLot(lotsData[0]?.id || "") // Default to the first lot
    } catch (error) {
  ...
  // Function to fetch slabs for the selected lot
  const fetchSlabsForLot = async (lotId: string) => {
    try {
      const slabsData = await getSlabByLotId(lotId) 
      if(slabsData)
        setVehicleTypes(slabsData)
    } catch (error) {
      console.error("Error fetching slabs for lot:", error)
    }
  }
  ...
  const handleAddSlab = () => {
    ...
    setVehicleTypes((prev) =>
      prev?.map((vt) => (vt.id === selectedVehicleType ? { ...vt, slabs: [...vt.slabs, newSlab] } : vt))??null
    )
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {
          !vehicleTypes || !selectedVehicleType || !currentVehicleType ?(
            <p className="text-center text-sm text-muted-foreground">
              No vehicle types available. Please add a vehicle type to start configuring rates.
            </p>
          ) : 
          <Card>
          <CardHeader>
        ...
        {/* Existing Vehicle Type Selector and Slab Configuration */}
        {/* ...existing code for vehicle type selector and slab configuration... */}
      </CardContent>
      <CardFooter className="flex justify-between">
        <Button variant="outline">Reset Changes</Button>
        <Button onClick={handleSaveChanges} disabled={!hasChanges}>
